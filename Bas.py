...
async def on_message(message):
        ...
        if msgsplit[0] == '$change' and not (gamestage == 'none' or gamestage == 'setup'):
            if msgsplit[1] == 'roundtime':
                ...
                if is_int(msgsplit[2]):
                    ...
                    roundmax = int(msgsplit[2])
                else:
                    await client.send_message(message.channel, 'Bitte gib eine ganze Zahl ein.')
            else:
                print('nomoreshittoprint')
    ...
    if gamestage == 'roundend' and message.author == client.user:
        ...
        desc = 'Die nächste Runde startet in 10 Sekunden, die Antworten werden in Direktnachrichten mit dem Bot geschrieben.'
        em = discord.Embed(title='Stadt, Land, Fluss', description=desc)
        await client.send_message(message.channel, embed=em)
    ...
    if gamestage == 'playing':
        ...
        answers[userindex][prefix-1] = ' '.join(msgsplit[1:len(msgsplit)])
        print(answers)
    ...
    # script for joining
    if msgsplit[0] != 'Spiel' and msgsplit[0] != 'Klicke' and message.author == client.user and gamestage == 'join':
    ...
    if msgsplit[0] == '$setup' and gamestage == 'none':
        ...
        if roundmax is None:
            ...
            await client.send_message(message.channel, 'Rundenanzahl erfolgreich auf {} gesetzt.'.format(roundtime))
